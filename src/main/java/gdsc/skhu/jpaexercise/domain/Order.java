...
@AllArgsConstructor
public class Order {
    @Id @GeneratedValue @Column(name = "`ORDER`_ID") // @ID는 PK / @GeneratedValue = 키 자동 증가 / @Column = 열을 의미
    private int id;

    @Column(name = "`ORDER`_NAME", nullable = false)  // consumer name / 주문자 이름
    private String name;

    @OneToMany(mappedBy = "order")  // Product클래스에서 order를 참조 변수로 사용하였기 때문에, order로 매핑함.
    @Column(name = "`ORDER`_LIST")   // 양방향 연관 관계는 사실 단방향이 쌍으로 이뤄진 것.
